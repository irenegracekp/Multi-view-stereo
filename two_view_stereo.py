...
def rectify_2view(rgb_i, rgb_j, R_irect, R_jrect, K_i, K_j, u_padding=20, v_padding=20):
    ...
    rgb_i_rect = cv2.warpPerspective(rgb_i, Hi, (w_max, h_max))
    rgb_j_rect = cv2.warpPerspective(rgb_j, Hj, (w_max, h_max))
...
def compute_rectification_R(T_ji):
    ...
    Tx = T_ji[0, 0]
    Ty = T_ji[1, 0]
    norm_val = math.sqrt(Tx**2 + Ty**2) + EPS
    r1 = np.zeros((3,1))
    r1[0,0] = Ty/norm_val
    r1[1,0] = -Tx/norm_val
...
def ssd_kernel(src, dst):
    ...
    for i in range(M):
      for j in range(N):
        ...
        result = list(map(sum, diff_square))
        ssd[i,j] = sum(result)
        ...
        for k in range(-(k_size//2), k_size//2 + 1):
          ...
          for l in range(-(k_size//2), k_size//2 + 1):
            ...
            if idy < 0 or idy >= w or 0 > idx or idx >= h:
              ...
              patch_buffer[i,j, count, :] = image[idx, idy, :]
            count += 1
